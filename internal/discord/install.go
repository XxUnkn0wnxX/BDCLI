...
func (discord *DiscordInstall) InstallBD(options models.InstallOptions) error {
	...
	legacyStates, err := preflightLegacyInjection(discord.Channel)
	if err != nil {
		return err
	}
	...
	if err := runLegacyInstallAction(legacyStates, func() error { return discord.inject(bd) }); err != nil {
		return err
	}
	output.Println("✅ Injection successful")
	if len(legacyStates) > 0 {
		output.Println("🔁 Migrated legacy BetterDiscord loader")
	}
	output.Blank()
...
func (discord *DiscordInstall) UninstallBD(options models.UninstallOptions) error {
	...
	if err := runLegacyRemovalAction(legacyStates, discord.uninject); err != nil {
		return err
	}
	if len(legacyStates) > 0 {
		output.Println("✅ Removed legacy BetterDiscord loader")
	}
	output.Blank()
