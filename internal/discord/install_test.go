...
func TestUninstallBD_UninjectOnly(t *testing.T) {
	...
	// Use an invalid channel so Exe() is empty and this high-level test cannot
	// match or stop a real Discord process; legacy preflight is inert for it.
	install := &DiscordInstall{ResourcesPath: resources, Channel: models.DiscordChannel(-1)}
